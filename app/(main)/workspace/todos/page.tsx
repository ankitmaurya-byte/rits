...
import { Plus, Users, Circle, Clock, CheckCircle2, MoreHorizontal } from "lucide-react";
...
export default function WorkspaceTodosPage() {
  ...
  const handleCreate = async (status: string) => {
    ...
    try {
      await createTodo({ scope: "workspace", workspaceId: selectedWorkspaceId, title: newTitle.trim(), priority: "medium", status, createdBy: convexUser?._id });
    ...
    <div className="page-container animate-fade-in-up relative h-full flex flex-col max-w-none overflow-hidden">
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners}
        ...
        <div className="flex gap-6 overflow-x-auto pb-8 flex-1 items-start relative z-10 min-h-[500px]">
          {STATUSES.map((status) => (
            <KanbanColumn key={status.id} status={status} tasks={tasksByStatus[status.id as keyof typeof tasksByStatus]}
              creatingInStatus={creatingInStatus} setCreatingInStatus={setCreatingInStatus}
              newTitle={newTitle} setNewTitle={setNewTitle} handleCreate={handleCreate}
              deleteTodo={deleteTodo} handleUpdateStatus={handleUpdateStatus} />
          ))}
        </div>
        ...
        {creatingInStatus === status.id ? (
          <div className="feature-card p-3 rounded-xl border shadow-sm" style={{ backgroundColor: "var(--surface-card)", borderColor: "var(--accent-blue)" }}>
            <input autoFocus value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
              ...
              onBlur={() => handleCreate(status.id)} placeholder="Task title..."
